...
import { NETLIFY_FUNCTIONS_CONFIG } from '../../apps/web-classic/src/config/netlifyConfig';
...
Prompt optimizado:`;
  ...
  const response = await fetch(NETLIFY_FUNCTIONS_CONFIG.externalAPI.primary + '/chat/completions', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${NETLIFY_FUNCTIONS_CONFIG.externalAPI.apiKey}`
    },
    body: JSON.stringify({
      model: NETLIFY_FUNCTIONS_CONFIG.env.QWEN_MODEL,
      messages: [
        {
          role: 'user',
          content: prompt
